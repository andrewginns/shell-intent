...
function Test-ShellIntentNaturalLanguage {
    ...
    $firstWord = ($trimmedLine -split '\s+', 2)[0]
...
function Get-ShellIntentInputDisposition {
    ...
    if (Test-ShellIntentNaturalLanguage -Line $Line) {
        return 'codex'
    }
...
function Enable-ShellIntent {
    ...
    Set-Alias -Name __shell_intent -Value Invoke-ShellIntentPendingQuery -Scope Global

    Set-PSReadLineKeyHandler -Chord Enter -BriefDescription 'Codex Terminal Bridge' -ScriptBlock {
        param($key, $arg)
        ...
        switch (Get-ShellIntentInputDisposition -Line $line) {
            'codex' {
                ...
                [Microsoft.PowerShell.PSConsoleReadLine]::Insert('__shell_intent')
                [Microsoft.PowerShell.PSConsoleReadLine]::AcceptLine($key, $arg)
                break
            }
...
function Disable-ShellIntent {
    ...
    Remove-Item alias:__shell_intent -ErrorAction SilentlyContinue
    Remove-Variable -Scope Global -Name ShellIntentPendingQuery -ErrorAction SilentlyContinue
}
